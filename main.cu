...
int main() {
    ...
    img_2d = (int **)malloc(width * sizeof(int *));
    ...
    //Normalize -> values between 0 and 1. 1D-ARRAY
    ...
    float *d_dense_2,*d_weights_2, *d_dense_3,*d_weights_3, *d_softmax;
    ...
    //Initialization
    initializeWithRandomValues(raw_data, width, height, inputChannels_C1);
    //initializeWithRandomValues(C1_data,width_C1,height_C1,outputChannels_C1);
    ...
    initializeWeights("layer_weights_0.bin", C1_kernel, filterSize*filterSize*outputChannels_C1);
    ...
    //initializeWithRandomValues(C2_kernel, filterSize, filterSize, outputChannels_C2);
    initializeWeights("layer_weights_2.bin", C2_kernel, filterSize*filterSize*outputChannels_C2*outputChannels_C1);

    //initializeWithRandomValues(weights_1, input_dense1, output_dense1, 1);
    initializeWeights("layer_weights_5.bin", weights_1, input_dense1*output_dense1);
    initializeWithZero(dense_1,output_dense1,1,1);

    initializeWeights("layer_weights_6.bin", weights_2, output_dense1*output_dense2);
    //initializeWithRandomValues(weights_2, output_dense1, output_dense2, 1);
    initializeWithZero(dense_2,output_dense2,1,1);

    initializeWeights("layer_weights_7.bin", weights_3, output_dense2*output_dense3);
    //initializeWithRandomValues(weights_3, output_dense2, output_dense3, 1);
    initializeWithZero(dense_3,output_dense3,1,1);
    initializeWithZero(sofmax_layer,output_dense3,1,1);
    //printMatrix(raw_data, width, height, inputChannels_C1);
    ...
    cudaMalloc(&d_softmax, output_dense3* sizeof(float));
    cudaMemcpy(d_softmax, sofmax_layer, output_dense3* sizeof(float), cudaMemcpyHostToDevice);
    ...
    conv2DKernelCombined<<<numBlocks_C2, threadsPerBlock_C2>>>(d_S1_data, d_C2_data, d_C2_kernel, width_S1, filterSize, outputChannels_C2,false,true);
    ...
    dim3 numBlocks_d1((input_dense1 + threadsPerBlock_d1.x - 1) / threadsPerBlock_d1.x);
    ...
    dim3 numBlocks_d2((output_dense1 + threadsPerBlock_d2.x - 1) / threadsPerBlock_d2.x);
    ...
    dim3 numBlocks_d3((output_dense2 + threadsPerBlock_d3.x - 1) / threadsPerBlock_d3.x);
    ...
    if (cudaStatus != cudaSuccess) {
        fprintf(stderr, "conv2DKernel launch failed: %s\n", cudaGetErrorString(cudaStatus));
    }


    cudaMemcpy(dense_3, d_dense_3, output_dense3 * sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(C1_kernel, d_C1_kernel, filterSize * filterSize * outputChannels_C1 * sizeof(float), cudaMemcpyDeviceToHost);

    //softmax(dense_3,  output_dense3);

    printMatrix(dense_3, output_dense3, 1, 1);

    int size = sizeof(dense_3);  
